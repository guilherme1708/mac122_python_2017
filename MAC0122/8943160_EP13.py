...
def main():
    ...
    e substitui o valor do par de ep's associado M[lin][col] por 2.0, além disso
    verifica se um par de ep's, se eles são iguais sem os comentários e também
    sem espaços em branco, substitui o valor do par de ep's associado M[lin][col] 
    por 3.0, por fim verifica se um par de ep's são iguais sem os comentários
    e sem espaços em branco  conferindo a troca de nome de variáveis e substitui
    o valor do par de ep's associado M[lin][col] por 4.0 e finalmente imprime 
    todos os ep's que foram considerados plágio, ou não.
    ...
    lista_aux1 = [] # Lista com os aqruivos sem os comentários e brancos
    lista_aux2 = [] # Lista com os aqruivos sem os comentários e brancos e troca de variaveis
    M = crie_matriz(n,n)
    cont = 0
    ...
    # Le todos os arquivos remove os comntários e armazena em uma lista
    for i in range(n):
        candidato = remove_brancos(lista_aux[i])
        lista_aux1.append(candidato)
           
    # Varre a matriz e verifica se um par de ep's canditados a plágio
    # e compara se são exatamente iguais, sem os comentários e sem espaços em branco
    for lin in range(n):
        for col in range(lin+1,n):
            if M[lin][col] != 1 and M[lin][col] != 2:
                candidato1 = lista_aux1[lin]
                candidato2 = lista_aux1[col]  
                if candidato1 == candidato2:
                    M[lin][col] = 3.0 
    
    # Le todos os arquivos remove os comntários e armazena em uma lista
    for i in range(n):
        candidato = troca_variaveis(lista_aux1[i])
        lista_aux2.append(candidato)
    
    # Varre a matriz e verifica se um par de ep's canditados a plágio
    # e compara se são exatamente iguais, sem os comentários, sem espaços em branco
    # verificando a troca de nome de variáveis
    for lin in range(n):
        for col in range(lin+1,n):
            if M[lin][col] != 1.0 and M[lin][col] != 2.0 and M[lin][col] != 3.0:
                candidato1 = lista_aux2[lin]
                candidato2 = lista_aux2[col]
                if candidato1 == candidato2:
                    M[lin][col] = 4.0 
...
def remove_comentarios(candidato):
    ...
    Recebe um aquivo candidato, removendo todos os comentarios retornando
    uma lista contendo só o código sem comentários.
    '''
...
# ------------------------------------------------------------
def remove_brancos(candidato):
    ''' (arquivo.py) -> lista
    
    Recebe um aquivo candidato ja sem os comentários e remove todos "brancos"  
    como espaços, tabs, newlines, etc, retornando uma lista contendo só o código
    sem brancos.
    '''
    ...
    # Laço que remove os espaços no inicio e fim de cada linha
    for line in candidato:
        novo = line.strip()
        lista_auxiliar.append(novo)

    # laço que remove "new lines"
    while VAZIO in lista_auxiliar:
        lista_auxiliar.remove(VAZIO)
    
    # Laço que remove espaços superflúos no meio do cógido
    for line in lista_auxiliar:
        novo = line.replace(' ','')
        sem_brancos.append(novo)
    
    return sem_brancos
# ------------------------------------------------------------
def troca_variaveis(candidato):
    ''' (arquivo.py) -> lista
    
    Recebe um aquivo candidato ja sem comentários e "bancos" e  
    verifica códigos disfarçados por troca de nomes variáveis
    retornando uma lista contendo o esqueleto do código
    '''
    so_var = []
